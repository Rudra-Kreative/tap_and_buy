...
$(document).on('click', '.deleteOwner', function () {
    Swal.fire({
        title: 'Are you sure to delete this Business owner?',
        text: "All the businesses owned by this owner will be deleted!",
        icon: 'warning',
        showCancelButton: true,
        ...
        if (result.isConfirmed) {
            ...
            $.ajax({
                type: "POST",
                url: $(this).closest('table').attr('data-target') + '/' + $(this).closest('td').attr('data-ownerId') + '/delete',
                data: { '_method': 'delete' },
                dataType: "json",
                beforeSend: function () {
                    $('#spinner-loader').fadeIn(100);
...
//suspend
$(document).on('click', '.suspendOwner', function () {

    Swal.fire({
        title: 'Are you sure to suspend this owner?',
        text: "All the businesses owned by this owner will be suspended!!",
        icon: 'warning',
        showCancelButton: true,
        confirmButtonColor: '#3085d6',
        cancelButtonColor: '#d33',
        ...
        if (result.isConfirmed) {
            ...
            $.ajax({
                type: "POST",
                url: $(this).closest('table').attr('data-target') + '/' + $(this).closest('td').attr('data-ownerId') + '/suspend',
                data: {'_method': 'put'},
                dataType: "json",
                beforeSend: function () {
                    $('#spinner-loader').fadeIn(100);
...
function preapreOwnerTable(data) {
    ...
    $.each(data, function (k, v) {
        ...
        tableBody += '<i class="fa fa-ban suspendOwner" title="Suspend" style="cursor: pointer" aria-hidden="true"></i>';
        tableBody += "</td></tr>";
    });
